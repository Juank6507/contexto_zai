...
| 12 | 5.0 | 2026-09-20 | plan_refactorizacion_v5.0.md | 230 |
| 13 | 6.0 | 2026-09-25 | plan_refactorizacion_v6.0.md | 538 |
...
- **v4.5** — Fases F1-F4: índice tema_a_archivo multi-bloque (tema→varios archivos) + registrar_tema() arreglado (no revienta) + 03_objetivo_proyecto.md ampliado (quién es + norte + qué pasó y cómo recuperarse) + eliminación de 04_resumenes_bloques.md y ResumenesGenerator + query_context lee resúmenes directamente de bloques físicos + orden de lectura 03 primero
- **v5.0** — Fases F1-F4: interfaces de plataforma (PlatformAuth, PlatformChat, PlatformLinks, PlatformLLM) + implementaciones de Z.ai + proxy de subagentes (servicio que llama al LLM directamente sin pasar por el agente) + conexión opt-in del proceso al proxy + tests E2E con medición de latencia
- **v6.0** — Fases F1-F7 divididas en dos bloques. Limpieza (F1-F3, implementadas): BlockPacker.pack() reparte temas grandes sin ValueError + RecoveryMetadata.tema_a_archivo dict[str, list[str]] multi-bloque + registrar_tema() idempotente + query_context/ProcesadorConsulta multi-bloque + EstadoGenerator 4 fixes reaplicados D1/A1/A2/A3 filtran intercambios virtuales y tool_calls + ContentCleaner._parse_tool_calls() formatea tool_calls como bloques legibles. Cascada (F4-F7, en implementación): Worker Bun persistente en puerto 8090 + Worker 1 lee bloque completo y escribe resumen al inicio del bloque + Workers 2-4 en paralelo sobre el resumen [nombre, decisiones, temas] + Integrador reformado que aplica las 4 respuestas juntas + consulta multi-bloque + tests E2E y medición de latencia (actual)
