...
- **v4.4** — Fases F1-F6: lógica de decisión de 4 casos (mismo chat vs otro chat) en Orchestrator, soporte multi-chat en metadata, IncrementalCycle arreglado (reempaquetado selectivo + regenera índice + acepta share_id), 3 sistemas de indexado conectados (temas→bloques + resúmenes + índice legible), subagentes de calidad cableados por defecto, tests E2E (actual)
